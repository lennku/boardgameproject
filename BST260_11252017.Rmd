...
Summary of sample sizes: 4665, 4664, 4664, 4663, 4665, 4665, ... 
Resampling results across tuning parameters:

  k  RMSE       Rsquared   MAE      
  5  0.5018103  0.2290551  0.3942637
  7  0.4914147  0.2467459  0.3862379
  9  0.4868664  0.2549673  0.3832619
...
#X <- game %>% select(-game_id) %>% slice(inTrain) %>% as.matrix

g <- game %>% filter(!is.na(weight))
...
#tmp = sample_n(train_set,500)

control <- trainControl(method='cv', number=20)
res <- train(avg_rating ~ .,
             data = train_set,
             method = "knn",
...
fit <- knnreg(avg_rating ~ ., train_set, k=3)
pred <- predict(fit, newdata = test_set)

tab <- table(pred, test_set$avg_rating)
confusionMatrix(tab)
